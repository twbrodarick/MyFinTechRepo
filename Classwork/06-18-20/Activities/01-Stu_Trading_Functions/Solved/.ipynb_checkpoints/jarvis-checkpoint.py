...
def initialize(cash=None):
    ...
    filepath = Path("MyFinTechRepo/Classwork/06-16-20/Activities/08-Ins_Trading_Dashboard/Resources/aapl.csv")
    
    initial_capital = float(100000)
    
    share_size = 500 

    return filepath, initial_capital, share_size

def fetch_data():
    """Fetches the latest prices."""
    aapl_df = pd.read_csv(filepath,
    parse_dates=True, 
...
def build_dashboard(tested_signals_df, portfolio_evaluation_df):
...
def main():
    """Main Event Loop."""
    filepath, initial_capital, share_size = initialize()
    aapl_df = fetch_data(filepath)
    signals_df = generate_signals(aapl_df)
    tested_signals_df = execute_backtest(signals_df, initial_capital, share_size)
    execute_trade_strategy()
    portfolio_evaluation_df = evaluate_metrics(tested_signals_df)
    dashboard = build_dashboard(tested_signals_df, portfolio_evaluation_df).dashboard.servable()
